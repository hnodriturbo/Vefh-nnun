...
def generate_genre_entry(genre_id, title, icon, item_type):
    ...
    return {
        ...
        'title': title,
        'icon': icon,
        'route': f"{item_type}/categories",
        'api_endpoint': f"/{item_type}/genre/{genre_id}"
    }
...
def create_category_listings(movie_categories, tv_categories):
    """ Creates a dictionary for each genre to be used in categories.html """
    category_listings = {
        'movie': {genre_id: {'title': details['title'], 'icon': details['icon']} for genre_id, details in movie_categories.items()},
        'tv': {genre_id: {'title': details['title'], 'icon': details['icon']} for genre_id, details in tv_categories.items()}
    }
