...
def show_listings(item_type, category, genre_id):
    """
    Displays listings of movies or TV shows based on category and optionally by genre.
    Args:
        item_type (str): 'movie' or 'tv', specifying the type of items.
        category (str): Type of listing ('popular', 'top_rated', 'genre').
        genre_id (int, optional): Genre ID for filtering listings.
    """
    if category not in ['popular', 'top_rated', 'genre']:
        flash('Invalid category specified', 'error')
    ...
    items = fetch_tmdb_data(endpoint, {'page': page})
    if items is None:
        flash('Failed to fetch items from the TMDB API', 'error')
        return redirect(url_for('home'))
    title = build_title(item_type, category, genre_id)
    return render_template('listings.html', items=items, title=title, listing_type=item_type, has_more_content=len(items) == 20)
